...
import { downloadMediaMessage } from './baileys/index.js';
import fs from 'fs';
...
import {
  ...
  getNextLaporanId, saveLaporan, uploadLaporanFoto,
  getGroupRouting,
...
import {
  sendMenuUtamaInteractive,
  sendPersyaratanInteractive,
  sendKategoriInteractive,
  sendKelurahanInteractive,
  sendWisataInteractive,
  extractInteractiveReply,
  WISATA_FALLBACK_TEXT
} from './menu-interactive.js';
...
export const handleMessage = async (sock, msg) => {
  ...
  const textMsg = (
    ...
    // List message reply (WhatsApp native list)
    actualContent.listResponseMessage?.singleSelectReply?.selectedRowId ||
    // Interactive/NativeFlow reply
    extractInteractiveReply(actualContent) ||
    ''
  ).trim();
  ...
  if (isGroup) {
    ...
    if (cmd === 'applylaporan') {
      ...
      const added = await addLaporanGroup(jid, groupName);
      if (added) {
    ...
    if (cmd === 'removelaporan') {
      const removed = await removeLaporanGroup(jid);
      if (removed) {
  ...
  // Alur Skrining IVA aktif
  if (session?.flow === 'iva') {
    await handleIvaFlow(sock, senderJid, pushName, session, textMsg, sendText);
    return;
  }
  ...
  if (isGreeting(textMsg) || textMsg === '0') {
    await sendMenuUtamaInteractive(sock, senderJid, pushName);
    return;
  }
  ...
  switch (textMsg) {
    case '1':
      await sendPersyaratanInteractive(sock, senderJid);
      break;

    case '2':
      clearSession(senderJid);
      setSession(senderJid, { flow: 'laporan', step: 'kategori' });
      await sendKategoriInteractive(sock, senderJid);
      logger.send(senderJid, 'Mulai alur pengaduan');
      break;

    case '3':
      await sendText(sock, senderJid, await buildKegiatanMenu());
    ...
    case '8':
      await sendWisataInteractive(sock, senderJid);
      break;
    ...
    case '11': {
      const statusMsg = await buildStatusLaporan(senderJid);
    ...
    case '10': {
      const existing = await getLivechatByJid(senderJid);
      if (existing) {
        ...
        await startLivechatSession(senderJid, pushName);
        setSession(senderJid, { flow: 'livechat' });
        await sendText(sock, senderJid,
          `💬 *LIVECHAT ADMIN*\n━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
    ...
    case 'iva': case 'IVA':
      ...
      logger.send(senderJid, 'Mulai Skrining IVA Test');
      break;
...
const handleLaporanFlow = async (sock, jid, name, msg, session, { textMsg, imageMsg, locationMsg }) => {
  ...
  switch (step) {
    ...
    case 'kategori': {
      ...
      if (!kategori) {
        await sendKategoriInteractive(sock, jid);
        return;
      }
      setSession(jid, {
        ...session,
      ...
      await sendKelurahanInteractive(sock, jid);
...
const handleLivechatFlow = async (sock, jid, name, msg, session, { textMsg, imageMsg }) => {
  ...
  if (textMsg === '0' || textMsg?.toLowerCase() === 'keluar') {
    await closeLivechatSession(jid);
    clearSession(jid);
    await sendText(sock, jid,
  ...
  if (imageMsg) {
    ...
    const result = await addLivechatMessage(jid, 'user', caption || '[Foto]', `/foto-livechat/${fname}`);
  ...
  const result = await addLivechatMessage(jid, 'user', textMsg);
  if (!result) {
    clearSession(jid);
    await sendText(sock, jid,
...
export { sendText, sendMenuUtamaInteractive as sendMenuUtama };
